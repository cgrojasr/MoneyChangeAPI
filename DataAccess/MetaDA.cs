...
using MoneyChangeAPI.Repositories;

namespace MoneyChangeAPI.DataAccess;

public class MetaDA : MetaRepository, CRUDRepository<MetaEntity>
{
    ...
    public IEnumerable<MetaItemModel> ListarPorUsuario(string id_usuario)
    {
        ...
        {
            using(conn){
                var query = $"SELECT MET.id_meta, MET.nombre, DET.porcentaje_avance, MET.url_image "+
                            $"FROM meta MET "+
